...
IDynamicsAero* AirVehicle::getDynamics()
{
   return dynamic_cast<IDynamicsAero*>( getDynamicsModel() );
}

const IDynamicsAero* AirVehicle::getDynamics() const
{
   return dynamic_cast<const IDynamicsAero*>( getDynamicsModel() );
}
...
{
   double value {};
   const IDynamicsAero* aero {getDynamics()};
   if (aero != nullptr) {
      value = aero->getGload();
   }
   return value;
...
{
   IDynamicsAero* aero {getDynamics()};
   if (aero != nullptr) {
