...
MXRP uses both bison (to generate parser) and math library from OpenSceneGraph.

----------------------------------- 
Based on OpenSceneGraph-3.0.1

MXRP is using just the osg math library (math, vec2, vec3, vec4, matrix and quat)

To keep this version from interfering with the full OSG packages, we made these changes ...
   1) wrapped in 'mxrp' namespace
